    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                <form method="POST"
                    action="{{ isset($pembayaran) ? route('pelaksana.projects.pembayaran.update', [$project->id, $pembayaran->id]) : route('pelaksana.projects.pembayaran.store', $project->id) }}">
                    @csrf
                    @if (isset($pembayaran))
                        @method('PUT')
                    @endif
                    ...
                    <div>
                        ...
                        <select id="jenis_pekerjaan_id" name="jenis_pekerjaan_id" required>
                            ...
                            @foreach ($jenisPekerjaanOptions as $id => $nama)
                                <option value="{{ $id }}"
                                    {{ old('jenis_pekerjaan_id', $pembayaran->jenis_pekerjaan_id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="jenis_transaksi" name="jenis_transaksi" required>
                            ...
                            <option value="Material"
                                {{ old('jenis_transaksi', $pembayaran->jenis_transaksi ?? '') == 'Material' ? 'selected' : '' }}>
                                Material</option>
                            <option value="Upah Tenaga Kerja"
                                {{ old('jenis_transaksi', $pembayaran->jenis_transaksi ?? '') == 'Upah Tenaga Kerja' ? 'selected' : '' }}>
                                Upah Tenaga Kerja</option>
                            <option value="Sewa Alat"
                                {{ old('jenis_transaksi', $pembayaran->jenis_transaksi ?? '') == 'Sewa Alat' ? 'selected' : '' }}>
                                Sewa Alat</option>
                            <option value="Jasa Konsultan"
                                {{ old('jenis_transaksi', $pembayaran->jenis_transaksi ?? '') == 'Jasa Konsultan' ? 'selected' : '' }}>
                                Jasa Konsultan</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input id="tanggal_pembayaran" type="date" name="tanggal_pembayaran"
                            value="{{ old('tanggal_pembayaran', $pembayaran->tanggal_pembayaran ?? date('Y-m-d')) }}"
                            required>
                    </div>
                    <div>
